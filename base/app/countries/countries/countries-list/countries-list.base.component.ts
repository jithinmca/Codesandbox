       ...
       const searchData = { ...this.getSearchData(this.filter.advancedSearch, this.tableSearchFieldConfig), ...this.getSearchData(this.filter.quickFilter, this.quickFilterFieldConfig) }
        ...
        if (this[property]) {
          ...
          this.isPageLoading = false;
		  	...
		  	this.countriesService.getDatatableData(params).subscribe((data: any) => {
				let updateRecords: CountriesBase[] = [...this.gridData, ...data?.results];
  ...
  if (this.selectedValues.length > 0) {
	...
	let requestedParams:any = {ids:values.toString()}
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.countriesService.delete(requestedParams).subscribe((res: any) => {
